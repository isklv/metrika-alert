...
const reportColumns = `id, counter_id, name, url_filter, url_match, goal_ids, group_by, period, enabled, created_at`

func scanReport(row interface{ Scan(...any) error }) (Report, error) {
	var r Report
	var goalIDs string
	err := row.Scan(&r.ID, &r.CounterID, &r.Name, &r.URLFilter, &r.URLMatch, &goalIDs, &r.GroupBy, &r.Period, &r.Enabled, &r.CreatedAt)
	if err != nil {
		return r, err
	}
...
func (db *DB) CreateReport(ctx context.Context, r *Report) error {
	res, err := db.ExecContext(ctx,
		`INSERT INTO reports (counter_id, name, url_filter, url_match, goal_ids, group_by, period, enabled)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		r.CounterID, r.Name, r.URLFilter, r.URLMatch, formatGoalIDs(r.GoalIDs), r.GroupBy, r.Period, r.Enabled,
	)
	if err != nil {
