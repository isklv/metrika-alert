...
### Сравнение по часу и дню недели

Раз в час сервис берёт закрывшийся час и сравнивает его с тем, что для этого
часа нормально:

```
вторник 15:00  ←→  прошлые вторники в 15:00 (по умолчанию 4 недели)
```
...
Почему не плоский порог: у трафика сильный суточный и недельный ритм. «Меньше
50 визитов в час» ночью — норма, днём — инцидент. Одно и то же число значит
разное в разные часы, поэтому сравнение всегда идёт с самим собой в прошлом.

### Когда час считается готовым

Час оценивается не сразу после закрытия: Метрика продолжает досчитывать сессии
ещё некоторое время после границы. Пауза задаётся `settle_minutes`
(по умолчанию 20). Час, который ещё идёт, не оценивается никогда — неполный час
на фоне полных выглядел бы падением каждый раз.

После простоя сервис догоняет пропущенные часы, но не более
`max_catch_up_hours` за одну проверку, чтобы не выдать пачку устаревших алертов
разом.
...
Все правила одного счётчика обслуживаются **одним запросом в час**: метрики
объединяются и запрашиваются вместе (лимит API — 20 метрик на запрос), а глубина
истории берётся по самому требовательному правилу. Десять правил на визиты не превращаются в
десять обращений к API.
...
| Алерты приходят с опозданием на час | так и задумано: час оценивается после закрытия и паузы `settle_minutes` |
