...
Полная форма правила добавляет два необязательных поля:
...
`мин_база` (по умолчанию 10) — порог значимости: если в этот час обычно бывает
...
Все правила одного счётчика обслуживаются **одним запросом на шаг**: метрики
объединяются и запрашиваются вместе (лимит API — 20 метрик на запрос), а глубина
истории берётся по самому требовательному правилу. Десять правил на визиты не
превращаются в десять обращений к API.
...
| Слишком много запросов к API | при шаге 10 минут это 144 запроса в сутки на счётчик. Увеличьте `step_minutes` |
