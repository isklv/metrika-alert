...
`--build` обязателен. Без него compose переиспользует уже собранный образ, и
`git pull` не даст ничего: сервис продолжит работать на старом коде. То же
касается `docker compose restart` — он только перезапускает контейнер, не
пересобирая его.

Проверить, что обновление доехало:

```bash
docker compose logs metrika-alert | head -1
# 2026-09-14 22:32:00 metrika-alert 2393e5c0bffe от 14.09.2026 22:28
```

Ту же строку показывает команда бота `/version`. Ревизию видно без всякой
настройки: образ собирается вместе с историей репозитория и проставляет её сам.
Пометка `+dirty` означает сборку из дерева с незакоммиченными правками.
`METRIKA_VERSION` задаётся только чтобы подписать релиз своим именем. Если команда из документации
не появилась в `/help` — сверьте версию: почти всегда это старый бинарь, а не
...
/schedule 10:00
   когда присылать отчёты: время суток, интервал (`6h`) или `off`

/addreport 1
→ Чекаут | url=/checkout | goals=42
   имя | какие страницы | какие цели
...
| `GET /api/report-defs?counter_id=N` | настроенные отчёты |
...
Для быстрого резервного копирования или переноса настроек без доступа к серверной файловой системе используйте команды бота:
- `/export` — выгружает все счётчики, правила, отчёты и расписание в JSON-файл (или `/export safe` без токенов).
- `/import` — восстанавливает настройки (в режиме `merge` или полной замены `/import replace`).
...
| Команда из документации отвечает «Неизвестная команда» и её нет в `/help` | запущен старый бинарь. Сверьте `/version` с `git rev-parse --short HEAD` и пересоберите: `docker compose up -d --build` |
...
| Отчёт приходит по всему сайту, а нужен по разделу | заведите отчёт с `url=`: `/addreport <id>` → `Чекаут \| url=/checkout`. Пока отчётов нет, присылается сводка по счётчику |
| Отчёты приходят слишком часто | `/schedule 10:00` — раз в сутки в 10:00. Настройка живёт в базе, перезапуск не нужен |
| Сводка по счётчику пропала после `/addreport` | так и задумано: настроенные отчёты заменяют её. Заведите отчёт без фильтров, если нужна и она |
